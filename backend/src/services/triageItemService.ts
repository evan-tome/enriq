...
export async function listTriageItems(workspaceId: string, status?: TriageStatus): Promise<TriageItemDto[]> {
...
export async function updateTriageItem(workspaceId: string, id: string, data: UpdateTriageItemBody): Promise<TriageItemDto> {
  const item = await prisma.triageItem.findFirst({ where: { id, workspaceId } });
  if (!item) throw new NotFoundError("Triage item not found");
  return toTriageItemDto(await prisma.triageItem.update({ where: { id }, data }));
}
